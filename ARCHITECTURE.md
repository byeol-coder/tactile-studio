...
- **15 commits**, **185 passing tests** (`npm test`), full TypeScript strict-mode typecheck (`npm run typecheck`), a working Vite 5.4 production build of the development shell (`npm run build:dev-shell`), and a working library-mode build (`npm run build:package`) — see [`MIGRATION.md`](./MIGRATION.md) for the phase-by-phase breakdown.
