...
// plain, browser-loadable module that assigns `window.DTMS_CORPUS`. No DB, no
// runtime fetch for corpus data. This is the "재료 준비" step: the app only
// depends on the generated `corpus.js`, never on the raw source folder.
//
...
Optional:
  --assets <dir>        Read approved Library Asset v1 JSON files (default: <src>/assets)
  --all                 Include all .dtms files in the source directory
  --include-drafts      Include draft/private assets for local review only`);
}
const srcArg = arg('--src');
const sourceInput = srcArg ?? process.env.DTMS_SRC;
if (argv.includes('--help') || argv.includes('-h')) {
  usage();
  process.exit(0);
}
if (!sourceInput) {
  console.error('✗ DTMS source folder is required.');
  usage();
  process.exit(1);
}
const SRC = resolve(sourceInput);
...
// Re-run: node scripts/build-corpus.mjs --src /path/to/dtms  (add files to CURATED or --all to grow)
// Static, backendless corpus bundle. Loaded via <script src="./corpus.js"> so it
// does not need a data API. Read at runtime as window.DTMS_CORPUS.
`;
